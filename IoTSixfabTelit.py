...
# from .SDL_Pi_HDC1000 import * # reading temperature
# from .MMA8452Q import MMA8452Q # read the accelerometer
...
class IoT():
    """docstring for ."""
    ip_address = "9.161.154.25"  # ip address
    domain_name = ""  # domain name
    port_number = "5000"  # port number
    response = ""  # variable for modem responses
    compose = ""  # variable for command composes
    timeout = TIMEOUT  # secondsvccvcxcxxfdg
    ...
    # Cellular Modes    # Telit: not checked yet
    AUTO_MODE = 0
    GSM_MODE = 1
    CATM1_MODE = 2
    CATNB1_MODE = 3
    ...
    def setupGPIO(self):
        ...
        # GPIO.setup(self.BG96_POWERKEY, GPIO.OUT)  # no in Telit
        # GPIO.setup(self.STATUS, GPIO.IN)  # no in Telit
    ...
    # def powerUp(self):    # removed for Telit
    ...
    # def getModemStatus(self):     # Telit commented as STATUS is commented
    ...
    def getResponse(self, desired_response):
        ...
        while 1:
            self.response = ""
            while(ser.inWaiting()):
                self.response += ser.read(ser.inWaiting()
    ...
    def sendATComm(self, command, desired_response, timeout=None):
        ...
        while 1:
            ...
            if(self.response.find(desired_response) != -1):
                debug_print(self.response)
                # returns the response of the command as string.
    ...
    def resetModule(self):
        ...
        delay(200)
        self.enable()
        # self.powerUp()    # Telit commented as powerUp() is commented
...
# *** TCP & UDP Protocols Functions ********************************************************
    ...
    def sendDataIFTTT(self, eventName, key, data):
        ...
        self.sendATComm(self.compose, "OK")
        self.clear_compose()
        url = str("https://maker.ifttt.com/trigger/" +
                  eventName + "/with/key/" + key)
        self.compose = "AT+QHTTPURL="
        self.compose += str(len(url))
        self.compose += ",80"
        self.setTimeout(20)
        ...
        self.clear_compose()
        self.sendDataComm(url, "OK")
        payload = "POST /trigger/" + eventName + "/with/key/" + key + \
