...
class IoTMqtt(IoTSixfabTelit.IoT):
    ...
    def mqtt_publish(self, message=None):
        ...
        self.new_sensor_data = json.dumps(dict(itertools.islice(sensor_data.items(), 6)))
        print("len(json.dumps(self.new_sensor_data))", len(self.new_sensor_data))
        ...
        self.sendATComm(f"AT#MQPUBS=1,\"5G-Solutions\",0,0,{self.new_sensor_data}"+self.CTRL_Z,"OK") 
